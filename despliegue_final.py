...
max_hr = st.slider('Frecuencia Cardíaca Máxima Alcanzada (latidos por minuto)', min_value=60, max_value=220, value=150, step=1)
oldpeak = st.slider('Depresión del Segmento ST inducida por el ejercicio (Oldpeak)', min_value=0.0, max_value=6.0, value=1.0, step=0.1)
fasting_bs = st.selectbox('¿El azúcar en sangre en ayunas es mayor a 120 mg/dl?', ['No', 'Sí'])
...
resting_ecg = st.selectbox('Resultado del electrocardiograma en reposo', ['Normal', 'Anormalidad de onda ST-T (ST)', 'Hipertrofia Ventricular Izquierda (LVH)'])
exercise_angina = st.selectbox('¿El paciente presenta angina inducida por el ejercicio?', ['No', 'Sí'])
